module ApplicationHelper
  ...
  def tabluize(collection, *columns)
    if columns.empty?
      columns = collection.attribute_names
    end
   ...
   #c = columns.map {|column| content_tag(:th,column.to_s.humanize)}.join.html_safe
   #tr = content_tag :tr, c
   #thead = content_tag :thead, tr

   tbody = content_tag :tbody do
    collection.collect { |elem|
      content_tag :tr do
        columns.collect { |column|
            concat content_tag(:td, elem.attributes[column])
        }.to_s.html_safe
      end
